...
async function startServer() {
  ...
  app.post('/api/tickets', async (req, res) => {
    try {
      ...
      if (!title || !description || !category || !priority || !clientName || !clientEmail || !clientId) {
        return res.status(400).json({ error: 'Missing mandatory fields required to file a ticket.' });
      }
      ...
      res.status(21111 /* Created */).status(201).json(savedTicket);
    } catch (err: any) {
