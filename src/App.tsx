...
import { User, Ticket, TicketStatus, TicketPriority, NotificationLog, UserRole, Attachment } from './types';
...
const DEMO_USERS: User[] = [
  { id: 'client-1', name: 'Jane Doe', email: 'jane.doe@workplace.com', role: 'client' },
  { id: 'client-2', name: 'John Smith', email: 'john.smith@co.com', role: 'client' },
  { id: 'agent-1', name: 'Alex Vance', email: 'alex.vance@workplace.com', role: 'agent' },
  { id: 'admin-1', name: 'Krzysztof Graczyk', email: 'krzysztof@bagietka.pl', role: 'admin' },
];
...
export default function App() {
  ...
  const [dbMode, setDbMode] = useState<'postgresql' | 'local-json-memory'>('local-json-memory');
  ...
  const handleUpdatePriority = async (ticketId: string, priority: TicketPriority) => {
    try {
      ...
      alert(`Urgency upgrade failed: ${err.message}`);
    }
  };
  ...
  const handleAssignTicket = async (ticketId: string, agentId: string, agentName: string) => {
    try {
      const resp = await fetch(`/api/tickets/${ticketId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ assignedTo: agentId, assignedName: agentName }),
      });
      if (!resp.ok) {
        const errorData = await resp.json();
        throw new Error(errorData.error);
      ...
      alert(`Personnel allocation failed: ${err.message}`);
    }
  };
  ...
  const handlePostMessage = async (ticketId: string, messageText: string, isInternal: boolean) => {
    try {
      ...
      alert(`Sending comment aborted: ${err.message}`);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col md:flex-row font-sans selection:bg-indigo-100 antialiased">
      ...
      <nav id="bento-desktop-sidebar" className="hidden md:flex w-24 bg-indigo-950 text-indigo-200 flex-col items-center py-6 gap-6 shrink-0 border-r border-indigo-900 justify-between">
        <div className="flex flex-col items-center gap-5 w-full">
          ...
          <div className="flex flex-col gap-3 w-full px-2">
            ...
            >
              ...
              <span className="text-[9px] uppercase tracking-wider font-bold">Zgłoszenia</span>
            </button>
            ...
            >
              ...
              <span className="text-[9px] uppercase tracking-wider font-bold">Nowe</span>
            </button>
            ...
            >
              <div className="relative">
                <span className="text-xl group-hover:scale-110 transition-transform">✉️</span>
                {logs.length > 0 && (
                  <span className="absolute -top-1 -right-2 bg-rose-500 text-white text-[8px] font-bold px-1 rounded-full">{logs.length}</span>
                )}
              </div>
              <span className="text-[9px] uppercase tracking-wider font-bold">E-maile</span>
            </button>
        ...
        <div id="bento-sidebar-bottom" className="flex flex-col items-center gap-3 w-full px-2">
          <div className="w-[60%] border-t border-indigo-900/40 my-1"></div>
          <div className="w-10 h-10 rounded-xl bg-indigo-900/50 border border-indigo-800 flex items-center justify-center text-[11px] font-bold text-indigo-100 font-mono tracking-wider select-none shrink-0" title={`Aktywny profil: ${currentUser.name}`}>
            {currentUser.name.split(' ').map(n => n[0]).join('')}
          </div>
          <span className="text-[7px] uppercase tracking-widest text-indigo-400 font-bold font-mono">{currentUser.role}</span>
        </div>
      </nav>

      {/* 2. Main Workspace Layout */}
      <div id="bento-workspace-root" className="flex-1 min-w-0 flex flex-col min-h-screen">
        {/* Testing Suite identity switcher (Modern Top Banner) */}
        <div id="bento-identity-switcher-banner" className="bg-slate-900 px-4 py-2 text-slate-305 border-b border-slate-950 text-xs flex flex-wrap justify-between items-center gap-3 z-30 shadow-inner">
          <div className="flex items-center gap-2">
            ...
            <span className="text-slate-400 font-medium">
              Zmień portal użytkownika, aby sprawdzić uprawnienia roli w czasie rzeczywistym:
            </span>
          </div>

          <div className="flex items-center gap-2">
            <label htmlFor="user-switch-select" className="sr-only">Wybierz portal</label>
            <select
        ...
        <div id="bento-mobile-header" className="md:hidden bg-indigo-950 px-4 py-3 pb-4 flex flex-col gap-3 border-b border-indigo-900 text-white z-20">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                <span className="font-bold text-sm tracking-tight block">Centrum Wsparcia IT</span>
                <span className="text-[10px] text-indigo-300 font-mono">Rola: {currentUser.role.toUpperCase()}</span>
              </div>
            </div>
          ...
          <div className="grid grid-cols-5 gap-1 text-center text-[9px] select-none font-bold">
            ...
            <button
              onClick={() => setActiveTab('submit')}
              className={`py-2 rounded-lg transition-colors cursor-pointer ${
                activeTab === 'submit' ? 'bg-indigo-600 text-white' : 'bg-indigo-900/50 text-indigo-300'
              }`}
            >
              ✏️ Nowe
            </button>
            ...
            <button
              onClick={() => setActiveTab('docker')}
              className={`py-2 rounded-lg transition-colors cursor-pointer ${
                activeTab === 'docker' ? 'bg-indigo-600 text-white' : 'bg-indigo-900/50 text-indigo-300'
              }`}
            >
              🐳 Docker
            </button>
        ...
        <header id="bento-top-header" className="bg-white border-b border-slate-200 py-6 px-6 md:px-8">
          <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              ...
              <p className="text-slate-550 text-xs mt-1">
                Witaj z powrotem, <strong className="text-slate-700 font-semibold">{currentUser.name}</strong> • Uprawnienia sektora: <span className="bg-slate-100 text-slate-700 px-1.5 py-0.5 rounded text-[10px] font-mono font-bold border border-slate-200 uppercase">{currentUser.role}</span>
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3 text-xs w-full sm:w-auto">
              ...
              <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold border ${
                ...
                <span>{wsConnected ? 'Połączenie aktywne' : 'WebSocket offline'}</span>
              </div>
              ...
              <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold bg-indigo-50 border border-indigo-200 text-indigo-850 shadow-2xs">
                ...
                <span>
                  Baza danych: <strong className="font-bold">{dbMode.toUpperCase()}</strong>
                </span>
              </div>
        ...
        <main id="bento-main-workspace" className="flex-1 max-w-7xl w-full mx-auto px-4 md:px-8 py-6 space-y-6">
          ...
          {apiError && (
            <div className="flex items-start gap-3 bg-rose-50 border border-rose-200 text-rose-800 text-xs rounded-2xl p-5 shadow-2xs hover:shadow-xs transition-shadow">
              ...
              <div className="space-y-1">
                <strong className="font-bold">Przerwa w połączeniu z bazą danych:</strong>
                <p className="leading-normal">{apiError} Sprawdź logi piaskownicy systemowej, aby zweryfikować stan synchronizacji schematu lub poświadczenia bazy.</p>
                <button
                  onClick={async () => {
                    setApiError(null);
                    setLoading(true);
          ...
          {loading ? (
            <div className="bg-white rounded-2xl shadow-xs border border-slate-205 py-36 text-center flex flex-col items-center justify-center">
              ...
              <p className="text-xs text-slate-500 font-semibold tracking-wide">Pobieranie klastrów systemowych. Proszę czekać...</p>
            </div>
          ) : (
            <div id="bento-inner-view-container" className="fade-in duration-300">
              {activeTab === 'dashboard' && (
                <TicketDashboard
                  tickets={tickets}
        ...
        <footer id="bento-footer" className="bg-slate-900 text-slate-400 border-t border-slate-950 py-6 text-center text-xs space-y-1.5 mt-auto">
          <p className="text-slate-300 font-sans font-medium tracking-wide">
            Panel Zarządzania Zgłoszeniami Wsparcia IT &copy; {new Date().getFullYear()} – bento panel
          </p>
          <div className="flex flex-wrap items-center justify-center gap-x-3 gap-y-1 text-[10px] text-slate-500">
            <span>Użytkownik: <strong className="text-slate-300">{currentUser.name}</strong></span>
            <span>•</span>
            <span>Rola dostępu: <span className="bg-slate-800 text-slate-300 px-1.5 py-0.5 rounded border border-slate-700 font-mono text-[9px] font-bold uppercase">{currentUser.role}</span></span>
            <span>•</span>
            <span>Środowisko: <span className="text-indigo-400 font-mono">v22.x</span></span>
          </div>
        </footer>
      </div>
    </div>
